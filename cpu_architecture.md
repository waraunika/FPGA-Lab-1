...
## Control Unit
  - initializes
	  ...
	  - `carry_use` as 0
	  - `DMUX_SEL` as 0
	  - `jmp_enable`, `call_enable`, `ret_enable` as 0
  ...
  - explanation of all signals, and their use:
	  ...
	  - `MUX1/2/3_SEL` to select for operand logic.
		  ...
		  - `MUX3`: 00 to select between `ALU`'s output, 01 to direct register transfer from `READREG1` and 10 `IMMEDIATE` databyte.
	  - flags:
		  ...
		  - `jmp_enable`, `call_enable`, `ret_enable`: flags to control how PC and stack should get its new corresponding values, if needed.
...
## 8x8 Register File
  ...
  - 2 as special register
	  ...
	  - reg\[7\] is not currently used, but can be used for XCHG operations, if implemented, and other operations when a temporary register is needed.
---
## 2's Comp
  - no need for clock
  - takes MUX2_OUT and 2's complements it.
---

## MUX1
  - based on select line `MUX1_SEL`, chooses `MUX2_OUT` or `~` from 2's Comp block
  - no need for clock
  - output as `COMP_OUT`
---

## MUX2
  - based on select line `MUX_SEL2`, choose between `REGOUT2` and `IMMEDIATE[7:0]`
  - output as OPERAND2\[7:0\]
  - no need for clock
---
## MUX3
- based on select line `MUX_SEL3`, choose between `ALURESULT`, `REGOUT1`, and `IMMEDIATE` to write into register file
---
## ALU
...
implemented: 110
future works: 17
total instructions planned: 127
...
2. JNZ DB: `11110 010`: F2H
3. JZ DB: `11110 011`: F3H
4. JNC DB: `11110 100`: F4H
5. JC DB: `11110 101`: F5H

6. CALL DB: `11111 000`: F8H
7. CNZ DB: `11111 010`: FAH
8. CZ DB: `11111 011`: FBH
9. CNC DB: `11111 100`: FCH
10. CC DB: `11111 101`: FDH

11. RET: `00010 000`: 10H
12. RNZ: `00010 010`: 12H
13. RTZ: `00010 011`: 13H
14. RNC: `00010 100`: 14H
15. RTC: `00010 101`: 15H
---
